    ...
    Friend Class ColumnLineHelper
        ...
        Public Property Columns As List(Of GridColumn)
            ...
            Set(ByVal value As List(Of GridColumn))
                If columnList Is value Then Return
                columnList = value
                grid.Refresh()
            End Set
        End Property
        ...
        Public Sub New(ByVal gridView As GridView)
            ...
            AddHandler grid.Paint, New Windows.Forms.PaintEventHandler(AddressOf grid_Paint)
            columnList = New List(Of GridColumn)()
            Hide = True
        End Sub

        Private Sub grid_Paint(ByVal sender As Object, ByVal e As Windows.Forms.PaintEventArgs)
            Dim vi As GridViewInfo = TryCast(view.GetViewInfo(), GridViewInfo)
            Dim standardPen As Pen = New Pen(view.PaintAppearance.Row.BackColor)
            Dim pen As Pen
            Dim p1 As Point = Point.Empty
            Dim p2 As Point = Point.Empty
            If Hide Then
                For Each ri As GridRowInfo In vi.RowsInfo
                    If ri.IsGroupRow Then Continue For
                    ...
                    For Each column As GridColumn In view.VisibleColumns
                        If Not columnList.Contains(column) Then Continue For
